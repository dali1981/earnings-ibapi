#!/usr/bin/env python3
"""
vol_term_v3.py — ATM implied‑vol term structure + skew with realized‑vol overlays

Key changes vs v2
-----------------
- Clean rewrite.
- **Greeks preference:** use IB streaming greeks when available; if you pass
  an `--as-of` in the past (or IB refuses greeks), we compute IV & delta
  from the option's price *at that time* using Black‑Scholes inversion.
- Uses repo partition key (`underlying`) when loading chains.
- Longer HMDS timeouts for reliability.

Run example:
  python vol_term_v3.py \
...
from ibx.greeks import compute_deltas_asof
...
def _load_latest_chain(chain_repo: ChainRepository, symbol: str) -> pd.DataFrame:
    """Load latest chain snapshot for symbol.
    Prefer partition key (underlying) to avoid full scans.
    """
    try:
        df = chain_repo.load(underlying=symbol)
    except TypeError:
        # older repo API: load everything
        df = chain_repo.load()
    if df.empty:
        raise RuntimeError(f"No chain snapshot found in repo (base={getattr(chain_repo, 'base_path', '???')}).")
    ...
    # Filter to symbol if not already
...
# -------------------- greeks: IB preferred, model fallback --------------------

def _option_greeks_prefer_ib(
    ...
    """Return (iv, delta). If `as_of` given, compute from bars at that time.
    Otherwise attempt IB streaming greeks and cancel promptly; fallback to
    computing from a near-now bar if IB refuses.
    """
    # Past as-of -> compute from bars
    if as_of:
        S_asof = _ref_price_asof(hist, subs, symbol, as_of)
        df = compute_deltas_asof(hist, symbol, expiry, [strike], right, as_of, S_asof, timeout=120.0)
        if not df.empty:
            iv = df.loc[0, "iv"] if 0 in df.index else df.iloc[0]["iv"]
            d  = df.loc[0, "delta"] if 0 in df.index else df.iloc[0]["delta"]
            return (None if pd.isna(iv) else float(iv)), (None if pd.isna(d) else float(d))

    # Try IB streaming greeks (genericTicks=106)
    try:
        ...
        try:
            for evt in h:
                if evt.get("type") == "tickOption":
                    if evt.get("impliedVol") is not None:
                        iv = float(evt["impliedVol"])  # 0.25 -> 25%
                    if evt.get("delta") is not None:
                        delta = float(evt["delta"])   # calls ~[0,1], puts ~[-1,0]
            ...
            h.cancel()
        if iv is not None or delta is not None:
            return iv, delta
    except Exception:
        pass

    # Fallback: compute from a near-now bar
    S_now = _ref_price_asof(hist, subs, symbol, None)
    df2 = compute_deltas_asof(hist, symbol, expiry, [strike], right, None, S_now, timeout=120.0)
    if not df2.empty:
        iv = df2.loc[0, "iv"] if 0 in df2.index else df2.iloc[0]["iv"]
        d  = df2.loc[0, "delta"] if 0 in df2.index else df2.iloc[0]["delta"]
        return (None if pd.isna(iv) else float(iv)), (None if pd.isna(d) else float(d))
...
def compute_realized_vol(
    ...
    """Close-to-close realized vol (annualized) over each lookback in days.
    Uses daily bars from IB (TRADES) and sqrt(252) * std(log returns).
    """
    if not lookbacks:
        return {}
    max_lb = max(lookbacks)
    duration = f"{max_lb + 40} D"  # pull extra
    end = ""
    if as_of:
        end = ib_end_datetime_instrument(hist.rt, make_stock(symbol), as_of, hyphen=True)
    rows = hist.bars(
...
def compute_atm_vol_term(
    ...
    *,
    ...
    for expiry in expiries:
        ...
        iv_c, d_c = _option_greeks_prefer_ib(subs, hist, symbol, expiry, k_atm, "C", as_of)
        iv_p, d_p = _option_greeks_prefer_ib(subs, hist, symbol, expiry, k_atm, "P", as_of)
...
def plot_term(df: pd.DataFrame, lookbacks: List[int], title: str | None = None, save: str | None = None):
    ...
    # HV overlays as horizontal lines
